...
if __name__ == '__main__':
    ...
    with open(r"/home/jy6ee/PZ_CO_ID/src/model_config.json", "r") as f:
        json_dict = json.load(f)
        PACKAGE_PATH = Path(json_dict["PACKAGE_PATH"])
        config = json_dict["model"]["CO_BC"]

    config.update({
        "input_csv_path_PDF": PACKAGE_PATH / 'data/relz_EPDF.csv',
        "input_csv_path_photz": PACKAGE_PATH / 'data/relz_EPDF.csv',
    ...
    # # relz starts here
    # config["input_csv_path_PDF"] = PACKAGE_PATH / 'data/relz_PDF11.csv'
    # config["input_csv_path_photz"] = PACKAGE_PATH / 'data/relz_LR.csv'
    # config["iteration"] = 1
    # config["CO_ratio"] = 0.35
    # config["weights"] = [[1, 2], [1, 15]]
    # for i in range(19):
    #     config["iteration"] += 1

    #     CO_BC_model = CO_BC(**config)
    ...
    # hsc starts here
    config["input_csv_path_PDF"] = PACKAGE_PATH / 'data/relz_PDF11.csv'
    config["input_csv_path_photz"] = PACKAGE_PATH / 'data/relz_EPDF.csv'
    config["iteration"] = 5
    config["CO_ratio"] = 0.1
    config["weights"] = [[1, 2], [2, 15]]
    for i in range(20):
        config["iteration"] += 1
